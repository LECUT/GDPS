...
class Obs_RINEX_Conversion(QMainWindow):
    global input_file_version
    def __init__(self, ratio):
        super().__init__()
        self.ratio = ratio
        self.setWindowTitle("observation data")
    ...
    def setup_ui(self):
        ...
        conb_box.addWidget(self.cheack300, 2, 3, 1, 1)
        conb_box.addWidget(self.cheack301, 3, 1, 1, 1)
        conb_box.addWidget(self.cheack302, 3, 2, 1, 1)
        conb_box.addWidget(self.cheack303, 3, 3, 1, 1)
        conb_box.addWidget(self.cheack304, 4, 1, 1, 1)
        conb_box.addWidget(self.cheack305, 4, 2, 1, 1)
        conb_box.addWidget(self.cheack400, 4, 3, 1, 1)
        conb_box.addWidget(self.cheack401, 5, 1, 1, 1)
    ...
    def check_version_button_group_change(self, id):
        try:
            ...
            if input_file_version == '2.11' or input_file_version == '2.12':
                if id == 211 or id == 212:
                    output_file_name = 'vc_' + head1 + '.' + tail1
                else:
                    output_file_name = head1[0:4].upper() + '00XXX_R_20' + tail1[0:2] + head1[4:7] + '0000_01D_30S_MO.rnx'
            else:
                if id == 211 or id == 212:
                    output_file_name = head1[0:4].lower() + head1[16:19] + head1[20:21] + '.' + head1[14:16] + 'o'
                else:
                    output_file_name = 'VC_' + head1[0:4].upper() + '00XXX_R_20' + tail1[0:2] + head1[4:7] + '0000_01D_30S_MO.rnx'
    ...
    def save_input_files_path_function(self):
        ...
        if os.path.exists(unzip_default_download_path):
            path1, parh2 = QFileDialog.getOpenFileName(self, 'Select the file', './', 'o_file(*MO.rnx *.*o *.*O *.obs);;All_File(*)')
        else:
            path1, parh2 = QFileDialog.getOpenFileName(self, 'Select the file', './',  'o_file(*MO.rnx *.*o *.*O *.obs);;All_File(*)')
        ...
        self.cheack212.setCheckable(True)
        ...
        self.cheack212.setEnabled(True)
        ...
        self.cheack212.setChecked(False)
            ...
            try:
                with open(path1, 'r') as f00:
                    ...
                    if first_line[20:36].upper() != 'OBSERVATION DATA':
                        QMessageBox.information(self, 'Error', 'The file is non-meteorological！')
                        return
                    global input_file_version
                    input_file_version = first_line[5:9].strip()
                    aaa = input_file_version
                    self.conf.set('Parameter', 'input_rinex_version', input_file_version)
                if input_file_version == '2.11':
                    self.cheack211.setEnabled(False)
                elif input_file_version == '2.12':
                    self.cheack212.setEnabled(False)
                elif input_file_version == '3.00':
                    self.cheack300.setEnabled(False)
                elif input_file_version == '3.01':
                    self.cheack301.setEnabled(False)
                elif input_file_version == '3.02':
                    self.cheack302.setEnabled(False)
                elif input_file_version == '3.03':
                    self.cheack303.setEnabled(False)
                elif input_file_version == '3.04':
                    self.cheack304.setEnabled(False)
                elif input_file_version == '3.05':
    ...
    def convert_function(self):
        ...
        self.conf.set('Parameter', 'output_file_path', self.show_outputsave_files_path_button.text())

        target_version = None
        if self.cheack211.isChecked():
            target_version = '2.11'
        elif self.cheack212.isChecked():
            target_version = '2.12'
        ...
        if target_version:
            try:
                ...
                if target_version == '2.11':
                    if input_file_version == '2.12':
                        convert_list = obs_212to211.OBS_RINEX212_to_RINEX211(input_path, target_version)
                    else:
                        convert_list = obs_3to2.OBS_RINEX3_to_RINEX2(input_path, target_version)
                elif target_version == '2.12':
                    if input_file_version == '2.11':
                        convert_list = obs_211to212.OBS_RINEX211_to_RINEX212(input_path, target_version)
                    else:
                        convert_list = obs_3to2.OBS_RINEX3_to_RINEX2(input_path, target_version)
                elif target_version == '3.00':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.01', '3.02', '3.03', '3.04', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto300.OBS_RINEX30X_to_RINEX300(input_path, target_version)
                elif target_version == '3.01':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.00', '3.02', '3.03', '3.04', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto301.OBS_RINEX30X_to_RINEX301(input_path, target_version)
                elif target_version == '3.02':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.00', '3.01', '3.03', '3.04', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto302.OBS_RINEX30X_to_RINEX302(input_path, target_version)
                elif target_version == '3.03':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.00', '3.01', '3.02', '3.04', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto303.OBS_RINEX30X_to_RINEX303(input_path, target_version)
                elif target_version == '3.04':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.00', '3.01', '3.02', '3.03', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto304.OBS_RINEX30X_to_RINEX304(input_path, target_version)
                elif target_version == '3.05' or target_version == '4.00' or target_version == '4.01':
                    if input_file_version == '2.11' or input_file_version == '2.12':
                        convert_list = obs_2to3.OBS_RINEX2_to_RINEX3(input_path, target_version)
                    elif input_file_version in ['3.00', '3.01', '3.02', '3.03', '3.04', '3.05', '4.00', '4.01']:
                        convert_list = obs_30xto305400.OBS_RINEX30X_to_RINEX305400(input_path, target_version)
            except:
            ...
            if self.box.clickedButton() == button_Look:
                self.s = Look_Over_File_details(self.show_outputsave_files_path_button.text(), self.ratio)
                self.s.show()
        else:
...
if __name__ == '__main__':
    ...
    print(dpi)
    ratio = dpi/96
    win = Obs_RINEX_Conversion(ratio)
    font = QFont()
