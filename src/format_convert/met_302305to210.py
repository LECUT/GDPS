...
#            <From 3.02/3.03/3.04/3.05 to 2.10>                  * |
# *                                                             * |
...
def MET_RINEX302305_to_RINEX210(input_file_path, target_version):
    ...
    for i in InputFile_type_observ_list:
        ...
        temp_num += 1
    inset_line_1 = ' ' * 5 + str(len(converd_type_observ_list)) + ' ' * 4 + '    '.join(str(j) for j in converd_type_observ_list)
    ...
    for row_line in iter_copy_raw_rinex_text_list:
        ...
        moment_time = ' ' + lines_text[3:20]
        record_data_list = re.findall('.{' + str(7) + '}', lines_text[20:])
        record_data_list.append(lines_text[20:][(len(record_data_list) * 7):])
        ...
        if all_type_observ_num != 1:
            ...
            add_second_line_data.append(lines_text[20:][(len(add_second_line_data) * 7):])
