...
#            <From 4.00 to 2.11/3.00/3.01/3.02/3.03/3.04/3.05>  * |
# *                                                             * |
...
def MET_RINEX4_to_RINEX211305(input_file_path, target_version):
    ...
    if target_version in ['2.11', '3.00', '3.01']:
        for row_line in iter_copy_raw_rinex_text_list:
            ...
            if len(lines_text[1:5].strip()) == 4:
                add_lines_text = ' ' + lines_text[3:]
            else:
                add_lines_text = lines_text
            converd_list.append([add_lines_text])
    elif target_version in ['3.02', '3.03', '3.04', '3.05']:
        for row_line in iter_copy_raw_rinex_text_list:
