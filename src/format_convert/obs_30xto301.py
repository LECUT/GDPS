...
#            <From 3.00/3.02/3.03/3.04/3.05/4.00 to 3.01>       * |
# *                                                             * |
...
def OBS_RINEX30X_to_RINEX301(file_path, target_version):
    raw_header_info = []
    ...
    print('RINEX-', input_file_version, ' Convert to RINEX-3.01')
    converted_save_list = []
    ...
    for deal_row in range(end_header_rows + 1):
        ...
        if header_label == 'RINEX VERSION / TYPE':
            add_dealed_list.append([current_row_list.replace(input_file_version, "3.01")])
    ...
    code_of_rine301_GPS_delete = ['C1R', 'L1R', 'D1R', 'S1R',
                                  'C2R', 'L2R', 'D2R', 'S2R']
    code_of_rinex301_Glonass = ['C1C', 'L1C', 'D1C', 'S1C',
                                'C1P', 'L1P', 'D1P', 'S1P',
                                'C2C', 'L2C', 'D2C', 'S2C',
                                'C2P', 'L2P', 'D2P', 'S2P']
    code_of_rinex301_BDS = ['C2I', 'L2I', 'D2I', 'S2I',
                            'C2Q', 'L2Q', 'D2Q', 'S2Q',
                            'C2X', 'L2X', 'D2X', 'S2X',
                            'C7I', 'L7I', 'D7I', 'S7I',
                            'C7Q', 'L7Q', 'D7Q', 'S7Q',
    ...
    converted_rinex301_obs_type_list = []
    converted_rinex301_obs_type_None_list = []
    for i in raw_rinex_obs_type_sort_01:
        temp_list = []
        temp_none_list = []
        if i[0] == 'G':
            converted_rinex301_obs_type_list.append(i)
            converted_rinex301_obs_type_None_list.append(i)
        elif i[0] == 'R':
            for j in i[1]:
                if j in code_of_rinex301_Glonass:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            converted_rinex301_obs_type_list.append([i[0], temp_list])
            converted_rinex301_obs_type_None_list.append([i[0], temp_none_list])
        elif i[0] == 'E':
            converted_rinex301_obs_type_list.append(i)
            converted_rinex301_obs_type_None_list.append(i)
        elif i[0] == 'S':
            ...
            converted_rinex301_obs_type_None_list.append([i[0], temp_none_list])
    finnal_insert_text = []
    for i in converted_rinex301_obs_type_list:
        temp_list = []
        if len(i[1]) <= 13:
            line_code = ''
            for j in i[1]:
            ...
            try:
                ...
                for j in raw_rinex_text_list[(now_deal_row):(now_deal_row + int(line[32:35]))]:
                    ...
                    if j[0] in ['G', 'R', 'E', 'S', 'C']:
                        ...
                        temp_num = 0
                        for i in converted_rinex301_obs_type_None_list:
                            if j[0] == i[0]:
                                for k in i[1]:
                                    if k:
