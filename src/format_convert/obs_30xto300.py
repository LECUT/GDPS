...
#            <From 3.01/3.02/3.03/3.04/3.05/4.00 to 3.00>       * |
# *                                                             * |
...
def OBS_RINEX30X_to_RINEX300(file_path, target_version):
    raw_header_info = []
    ...
    print('RINEX-', input_file_version, ' Convert to RINEX-3.00')
    converted_save_list = []
    ...
    for deal_row in range(end_header_rows + 1):
        ...
        if header_label == 'RINEX VERSION / TYPE':
            add_dealed_list.append([current_row_list.replace(input_file_version, "3.00")])
    ...
    code_of_rinex300_GPS_delete = ['C1S', 'L1S', 'D1S', 'S1S',
                                   'C1L', 'L1L', 'D1L', 'S1L',
                                   'C1X', 'L1X', 'D1X', 'S1X',
                                   'C1R', 'L1R', 'D1R', 'S1R',
                                   'C2R', 'L2R', 'D2R', 'S2R']
    code_of_rinex300_Glonass = ['C1C', 'L1C', 'D1C', 'S1C',
                                'C1P', 'L1P', 'D1P', 'S1P',
                                'C2C', 'L2C', 'D2C', 'S2C',
                                'C2P', 'L2P', 'D2P', 'S2P']
    converted_rinex300_obs_type_list = []
    converted_rinex300_obs_type_None_list = []
    for i in raw_rinex_obs_type_sort_01:
        temp_list = []
        temp_none_list = []
        if i[0] == 'G':
            for j in i[1]:
                if j not in code_of_rinex300_GPS_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            converted_rinex300_obs_type_list.append([i[0], temp_list])
            converted_rinex300_obs_type_None_list.append([i[0], temp_none_list])
        elif i[0] == 'R':
            for j in i[1]:
                if j in code_of_rinex300_Glonass:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            converted_rinex300_obs_type_list.append(i)
            converted_rinex300_obs_type_None_list.append(i)
        elif i[0] == 'S':
            converted_rinex300_obs_type_list.append(i)
            converted_rinex300_obs_type_None_list.append(i)
    finnal_insert_text = []
    for i in converted_rinex300_obs_type_list:
        temp_list = []
        if len(i[1]) <= 13:
            line_code = ''
            for j in i[1]:
            ...
            try:
                ...
                for j in raw_rinex_text_list[(now_deal_row):(now_deal_row + int(line[32:35]))]:
                    ...
                    if j[0] in ['G', 'R', 'E', 'S']:
                        ...
                        temp_num = 0
                        for i in converted_rinex300_obs_type_None_list:
                            if j[0] == i[0]:
                                for k in i[1]:
                                    if k:
