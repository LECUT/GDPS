...
#            <From 3.00/3.01/3.02/3.03/3.05/4.00 to 3.04>       * |
# *                                                             * |
...
def OBS_RINEX30X_to_RINEX304(file_path, target_version):
    raw_header_info = []
    ...
    print('RINEX-', input_file_version, ' Convert to RINEX-3.04')
    converted_save_list = []
    ...
    for deal_row in range(end_header_rows + 1):
        ...
        if header_label == 'RINEX VERSION / TYPE':
            add_dealed_list.append([current_row_list.replace(input_file_version, "3.04")])
    ...
    code_of_rine303_GPS_delete = ['C1R', 'L1R', 'D1R', 'S1R',
                                  'C2R', 'L2R', 'D2R', 'S2R']

    code_of_rine303_NavIC_delete = ['C1D', 'L1D', 'D1D', 'S1D',
                                    'C1P', 'L1P', 'D1P', 'S1P',
                                    'C1X', 'L1X', 'D1X', 'S1X']

    code_of_rine304_QZSS_delete = ['C1B', 'L1B', 'D1B', 'S1B']
    converted_rinex304_obs_type_list = []
    converted_rinex304_obs_type_None_list = []
    for i in raw_rinex_obs_type_sort_01:
        temp_list = []
        temp_none_list = []
        if i[0] == 'G':
            for j in i[1]:
                if j not in code_of_rine303_GPS_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            converted_rinex304_obs_type_list.append([i[0], temp_list])
            converted_rinex304_obs_type_None_list.append([i[0], temp_none_list])
        elif i[0] == 'C':
            if input_file_version == '3.02':
                ...
                BDS_temp_code = [m.replace('1S', '1A') if '1S' in m else m for m in i[1]]
                BDS_temp_code = [m.replace('1L', '1A') if '1L' in m else m for m in BDS_temp_code]
                BDS_temp_code = [m.replace('1Z', '1A') if '1Z' in m else m for m in BDS_temp_code]
                BDS_temp_code = [m.replace('6D', '6A') if '6D' in m else m for m in BDS_temp_code]
                BDS_temp_code = [m.replace('6P', '6A') if '6P' in m else m for m in BDS_temp_code]
                BDS_temp_code = [m.replace('6Z', '6A') if '6Z' in m else m for m in BDS_temp_code]
            ...
            for j in i[1]:
                if j not in code_of_rine304_QZSS_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            for j in i[1]:
                if j not in code_of_rine303_NavIC_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            converted_rinex304_obs_type_list.append(i)
            converted_rinex304_obs_type_None_list.append(i)

    finnal_insert_text = []
    for i in converted_rinex304_obs_type_list:
        temp_list = []
            ...
            try:
                ...
                for j in raw_rinex_text_list[(now_deal_row):(now_deal_row + int(line[32:35]))]:
                    ...
                    if j[0] in ['G', 'R', 'E', 'S', 'C', 'J', 'I']:
                        ...
                        temp_num = 0
                        for i in converted_rinex304_obs_type_None_list:
                            if j[0] == i[0]:
                                for k in i[1]:
                                    if k:
