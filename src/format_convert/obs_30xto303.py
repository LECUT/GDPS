...
#            <From 3.00/3.01/3.02/3.04/3.05/4.00 to 3.03>       * |
# *                                                             * |
...
def OBS_RINEX30X_to_RINEX303(file_path, target_version):
    raw_header_info = []
    ...
    print('RINEX-', input_file_version, ' Convert to RINEX-3.03')
    converted_save_list = []
    ...
    for deal_row in range(end_header_rows + 1):
        ...
        if header_label == 'RINEX VERSION / TYPE':
            add_dealed_list.append([current_row_list.replace(input_file_version, "3.03")])
    ...
    code_of_rine303_GPS_delete = ['C1R', 'L1R', 'D1R', 'S1R',
                                  'C2R', 'L2R', 'D2R', 'S2R']
    code_of_rine303_Glonass_delete = ['C4A', 'L4A', 'D4A', 'S4A',
                                      'C4B', 'L4B', 'D4B', 'S4B',
                                      'C4X', 'L4X', 'D4X', 'S4X',
                                      'C6A', 'L6A', 'D6A', 'S6A',
                                      'C6B', 'L6B', 'D6B', 'S6B',
                                      'C6X', 'L6X', 'D6X', 'S6X']
    code_of_rinex303_BDS = ['C2I', 'L2I', 'D2I', 'S2I',
                            'C2Q', 'L2Q', 'D2Q', 'S2Q',
                            'C2X', 'L2X', 'D2X', 'S2X',
                            'C7I', 'L7I', 'D7I', 'S7I',
                            'C7Q', 'L7Q', 'D7Q', 'S7Q',
                            ...
                            'C6Q', 'L6Q', 'D6Q', 'S6Q',
                            'C6X', 'L6X', 'D6X', 'S6X']
    code_of_rine303_QZSS_delete = ['C1B', 'L1B', 'D1B', 'S1B',
                                   'C5D', 'L5D', 'D5D', 'S5D',
                                   'C5P', 'L5P', 'D5P', 'S5P',
                                   'C5Z', 'L5Z', 'D5Z', 'S5Z',
                                   'C6E', 'L6E', 'D6E', 'S6E',
    ...
    code_of_rine303_NavIC_delete = ['C1D', 'L1D', 'D1D', 'S1D',
                                    'C1P', 'L1P', 'D1P', 'S1P',
                                    'C1X', 'L1X', 'D1X', 'S1X',]
    converted_rinex303_obs_type_list = []
    converted_rinex303_obs_type_None_list = []
    for i in raw_rinex_obs_type_sort_01:
        ...
        if i[0] == 'G':
            ...
            converted_rinex303_obs_type_list.append([i[0], temp_list])
            converted_rinex303_obs_type_None_list.append([i[0], temp_none_list])
        elif i[0] == 'R':
            for j in i[1]:
                if j not in code_of_rine303_Glonass_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            converted_rinex303_obs_type_list.append(i)
            converted_rinex303_obs_type_None_list.append(i)
        elif i[0] == 'S':
            converted_rinex303_obs_type_list.append(i)
            converted_rinex303_obs_type_None_list.append(i)
        elif i[0] == 'C':
            if input_file_version == '3.02':
                BDS_temp_code = [m.replace('1', '2') if '1' in m else m for m in i[1]]
            else:
                BDS_temp_code = i[1]
            for j in BDS_temp_code:
                if j in code_of_rinex303_BDS:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            for j in i[1]:
                if j not in code_of_rine303_QZSS_delete:
                    temp_list.append(j)
                    temp_none_list.append(j)
                else:
                    temp_none_list.append(None)
            ...
            converted_rinex303_obs_type_None_list.append([i[0], temp_none_list])
    finnal_insert_text = []
    for i in converted_rinex303_obs_type_list:
        temp_list = []
            ...
            try:
                ...
                for j in raw_rinex_text_list[(now_deal_row):(now_deal_row + int(line[32:35]))]:
                    ...
                    if j[0] in ['G', 'R', 'E', 'S', 'C', 'J', 'I']:
                        ...
                        temp_num = 0
                        for i in converted_rinex303_obs_type_None_list:
                            if j[0] == i[0]:
                                for k in i[1]:
                                    if k:
