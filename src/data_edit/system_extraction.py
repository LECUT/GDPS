...
# *                     All rights reserved.                    * |
# *                                                             * |
# * Brief    Extract file data based on satellite system        * |
# *                                                             * |
...
def System_Extraction_Function(input_file_content, SatelliteSystem_list):
    ...
    if len(SatelliteSystem_list) > 1:
        satellite_sysy_tab = 'M'
    elif len(SatelliteSystem_list) == 1:
        satellite_sysy_tab = SatelliteSystem_list[0]
    raw_header_record[0] = raw_header_record[0][:40] + satellite_sysy_tab + ' ' * 19 + raw_header_record[0][60:]
    delete_header_row_list = []
    for i in range(end_header_rows):
        line_text = raw_rinex_text_list[i].strip('\n')
        if line_text[60:80].strip() == 'SYS / # / OBS TYPES' and line_text[0] != ' ':
            if line_text[0] not in SatelliteSystem_list:
                delete_header_row_list.append(i)
                num = i + 1
                while raw_rinex_text_list[num].strip('\n')[:7] == '       ':
                    delete_header_row_list.append(num)
                    num += 1
    for i in reversed(delete_header_row_list):
        raw_header_record.pop(i)
        ...
        for line in iter_data_record_list:
            if len(line[:32].split()) == 8:
                ...
                for prn in copy_record_all_satellite_list:
                    if prn[0] not in SatelliteSystem_list:
        ...
        for line in iter_data_record_list:
            if len(line[:32].split()) == 8:
                ...
                for i in moment_prn_data_list:
                    if i[0] not in SatelliteSystem_list:
